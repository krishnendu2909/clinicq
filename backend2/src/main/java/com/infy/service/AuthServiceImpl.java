...
public class AuthServiceImpl implements AuthService {
 ...
    }
   ...
   @Override
   public void forgotPassword(String email) throws InfyHospitalException {
       User user=userRepository.findByEmail(email).orElseThrow(()->new InfyHospitalException("Service.INVALID_USER"));
       
...
//        return token;
 ...
        User user=userRepository.findByResetToken(token).orElseThrow(()->new InfyHospitalException("Service.INCORRECT_OTP"));
 ...
        userRepository.save(user);
   ...
   @Override
   public void verifyResetToken(String token) throws InfyHospitalException
   {
               User user=userRepository.findByResetToken(token)
                       .orElseThrow(()-> new InfyHospitalException("Service.INVALID_RESET_TOKEN"));
               
               if(user.getTokenExpiry()==null || user.getTokenExpiry().isBefore(LocalDateTime.now()))
               {
                   throw new InfyHospitalException("Service.TOKEN_EXPIRED");
