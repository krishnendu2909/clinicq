...
const BookAppointment: React.FC<BookProps> = ({ onNavigate }) => {
  ...
  useEffect(()=>{
    ...
    const fetchBookingRules=async()=>{
      ...
      try{
        ...
        const res=await axiosInstance.get('/clinicq/admin/booking-rules');

        if(res.data && res.data.maxDays){

          setMaxAdvanceDays(res.data.maxDays);

        }

      } catch(err){

        console.error("Failed to fetch booking rules");
  ...
  const handleCancel = async (id: number) => {
    ...
    // const deadline=new Date(eventStartTime.getTime()-(2*60*60*1000));
    ...
    //   return `Cancellation failed.You should have tried ${diffMinutes} mins earlier.`;

    // }else{

    //   const diffHours=(diffMinutes/60).toFixed(1);

    //   return `Cancellation failed. You should have tried ${diffHours} hours earlier.`;
    ...
    <div className="d-flex min-vh-100 overflow-hidden" style={{ background: 'radial-gradient(circle at center, #f8f9fa 0%, #e9ecef 100%)', color: '#212529', fontFamily: "'Segoe UI', Roboto, sans-serif" }}>
      ...
      <div className="bg-white border-end p-3 d-flex flex-column shadow-sm" style={{ width: '240px' }}>
        ...
        <div className="nav flex-column gap-2">
          ...
          <button onClick={() => onNavigate('book')} className="btn text-start py-2 px-3 border-0 fw-bold" style={{ background: 'rgba(255, 126, 95, 0.1)', color: '#ff7e5f', borderRadius: '12px' }}>📅 Book Appt</button>


 

          <button onClick={() => onNavigate('my')} className="btn text-start py-2 px-3 border-0 text-muted fw-medium">📋 My Appts</button>
            ...
            <div className="glass-card-light p-4 flex-grow-1 d-flex flex-column shadow-lg">
              ...
              <div className="row g-3 mb-4">
                ...
                <div className="col-6">
                  ...
                  <input type="date" className="form-control border-0 shadow-sm rounded-3 fw-bold bg-light" value={selectedDate} min={today} onChange={(e) => setSelectedDate(e.target.value)} />
              ...
              <div className="custom-scrollbar overflow-auto flex-grow-1 mb-3">
                ...
                <div className="d-flex flex-wrap gap-2">
                  ...
                  .filter(slot=>selectedDate>today || slot.startTime>new Date().toTimeString().substring(0,5))
          ...
          <div className="col-xl-6 d-flex flex-column h-100">
            ...
            <div className={`glass-card-light p-4 ${reschedulingId ? 'shadow-lg border-warning' : 'opacity-50'}`} style={{ border: reschedulingId ? '2px solid #ffc107' : '' }}>
              ...
              <div className="row g-2">
                ...
                <div className="col-6"><input type="date" className="form-control rounded-3" value={rescheduleDate} min={today} onChange={(e) => setRescheduleDate(e.target.value)} /></div>
                ...
                <div className="col-6">
                  ...
                  <select className="form-select rounded-3" disabled={!reschedulingId} value={rescheduleTime} onChange={(e) => setRescheduleTime(e.target.value)}>
